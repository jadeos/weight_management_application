  ...
  // //Add a food to the food log upon entry
   function insertIntoFoodLog($user_id,$food_id,$entry_type){
    ...
    $date = date('Y-m-d');
     ...
     $res=sqlQuery("INSERT INTO user_food_log (date_a, user_id, food_id, entry_type) VALUES (now(),'$user_id','$food_id', '$entry_type')");
    //echo "test";
   ...
   function calorie_summary($user_id){
    $res = sqlQuery("SELECT sum(fd.calories) `total_cals` from food_directory fd JOIN user_food_log l ON fd.food_id = l.food_id WHERE l.user_id ='$user_id' AND l.date_a = CURDATE();");
    return $res;
   }
}
